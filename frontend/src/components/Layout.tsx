...
const Layout = () => {
    ...
    return (
        <div className="flex h-screen bg-slate-100 dark:bg-black text-slate-900 dark:text-zinc-100 font-sans transition-colors duration-200">
            ...
            {isMobileMenuOpen &&
                <div
                    className="fixed inset-0 bg-slate-900/50 dark:bg-black/80 z-40 md:hidden"
            ...
            <div className="flex flex-col flex-1 overflow-hidden">
                ...
                <header className="h-16 bg-white dark:bg-zinc-900 border-b border-slate-200 dark:border-zinc-800 flex items-center justify-between px-4 md:px-8 gap-4 transition-colors">
                    <div className="flex items-center gap-4">
                        <button
                            className="md:hidden p-2 -ml-2 text-slate-500 dark:text-zinc-400 hover:text-slate-900 dark:hover:text-white"
                            onClick={() => setIsMobileMenuOpen(true)}
                        >
                            <MenuIcon className="h-6 w-6 text-slate-600 dark:text-zinc-300" />
                        </button>
                        <div>
                            <h1 className="text-slate-900 dark:text-white font-semibold text-[15px] sm:text-base">
                                {title}
                            </h1>
                            <p className="text-xs text-slate-400 dark:text-zinc-400 hidden sm:block">
