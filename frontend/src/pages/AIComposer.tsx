        ...
        <div className="max-w-4xl mx-auto space-y-12 pb-20 animate-in fade-in duration-700">
            ...
            <div className="space-y-6 text-center mt-20">
                ...
                <div className="relative group mt-12">
                    <textarea
                        className="w-full px-6 py-6 bg-white dark:bg-zinc-950 border border-slate-200 dark:border-zinc-800 rounded-xl text-slate-900 dark:text-white placeholder-slate-400 dark:placeholder-zinc-500 outline-none focus:border-red-400 dark:focus:border-red-500/50 transition resize-none h-40 shadow-xs"
                    ...
                    <div className="absolute bottom-4 right-2.5 flex items-center gap-3 text-sm">
                        <button
                            ...
                            className="flex items-center gap-3 bg-red-50 dark:bg-red-950/40 border border-red-100 dark:border-red-900/50 text-slate-700 dark:text-zinc-300 py-2 px-3 rounded-lg text-xs font-medium cursor-pointer"
                        >
                            <span>AI Image</span>
                            <div className={`relative inline-flex h-5 w-9 shrink-0 cursor-pointer rounded-full transition-colors duration-200 ease-in-out focus:outline-none ${generateImage ? "bg-red-500" : "bg-slate-200 dark:bg-zinc-700"}`}>
                        ...
                        <button onClick={handleGenerate} disabled={loading} className="bg-slate-900 hover:bg-slate-800 dark:bg-red-600 dark:hover:bg-red-500 text-white flex items-center gap-2 px-4 py-2 rounded-lg font-medium transition cursor-pointer disabled:opacity-50">
                            {loading ? (
                                <>
                ...
                <div className="flex flex-wrap justify-center gap-2">
                    {tones.map((t) => (
                        <button
                            key={t}
                            onClick={() => setTone(t)}
                            className={`px-4 py-1.5 rounded-full text-sm transition-all border cursor-pointer ${tone === t ? "bg-red-500 dark:bg-red-600 border-red-500 dark:border-red-600 text-white shadow-xs" : "bg-white dark:bg-zinc-950 border-slate-200 dark:border-zinc-800 text-slate-600 dark:text-zinc-400 hover:border-slate-300 dark:hover:border-zinc-700"}`}
                        >
                            {t}
                        </button>
                    ))}
